...
- A daily background job checks subscribed feeds for new episodes. A separate hourly job keeps the search index current.
...
Backend: FastAPI, PostgreSQL with pgvector, faster-whisper, sentence-transformers, rank-bm25, LangGraph, Claude (Haiku), APScheduler, OpenTelemetry, RAGAS.
...
|---|---|---|
| `/ask` | POST | Answer a question. Streams NDJSON: one citations event, then a sequence of token events. |
| `/podcasts` | GET | List subscribed podcasts with episode counts. |
| `/podcasts` | POST | Subscribe to a feed. Streams sync progress. Capped at 3 new episodes per call. |
| `/podcasts` | DELETE | Unsubscribe. Episodes stay in the database and remain searchable. |
| `/podcasts/backfill` | POST | Background job to fully process a large back catalog beyond the per-sync cap. |
...
**Running Whisper and the embedding model in the same process caused hard crashes.** CTranslate2 (used by faster-whisper) and torch's OpenMP thread pool collided, producing a segfault. Fixed by running transcription in its own subprocess. A second, similar looking crash turned out to have a different cause: a transitively installed newer NumPy silently broke torch's pinned binary compatibility. Fixed by pinning `numpy<2` explicitly in `requirements.txt`, rather than trusting whatever pip happens to resolve.

**Subscribing to a large podcast tried to process its entire back catalog at once.** One show had over 1,700 episodes in its feed. Fixed with a per-sync cap of 3 new episodes, with the daily scheduler working through any backlog gradually, and a separate backfill endpoint for anyone who wants the full history immediately.
...
- The per-sync cap means a high volume feed catches up slowly through the daily scheduler rather than all at once.
- Speaker diarization was intend but not built.